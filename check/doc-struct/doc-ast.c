...
Test(chec_ast_checks, correct_node_memory_cycle)
{
	...
	int rc = doc_ast_node_create(&docAst, WORD, nxt, prev, pnt, 0, an);
	cr_assert(rc == 0, "Failed to create new DocAst node\n");
	cr_assert(docAst.antype == WORD, "docAst type incorrectly set\n");
	...
	cr_assert(docAst.andata.word.wrd == an.word.wrd, "docAst type incorrectly set\n");
