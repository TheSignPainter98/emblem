...
Test(map, make_from_list)
{
	...
	for (size_t i = 0; i < NUM_MAP_TEST_ENTRIES; i++)
	{
		...
		cr_assert((size_t)r.just == i * i, "Retrieved wrong value from map, expected '%ld' but got '%ld'", i * i,
			(size_t)r.just);
		dest_maybe(&r, NULL);
	}

	cr_assert(m.curr_stored == l.cnt,
		"Map does not report the same number of entries as were entered, expected %d but got %d", NUM_MAP_TEST_ENTRIES,
		m.curr_stored);
...
Test(map, push_get_absent)
{
	...
	for (size_t i = 0; i < NUM_MAP_TEST_ENTRIES; i++)
	{
		...
		cr_assert((size_t)r2.just == i * i,
			"Value retrieved from map did not match that put in, expected %ld but got %ld", i * i, (size_t)r2.just);

		dest_maybe(&r, NULL);
		dest_maybe(&r2, NULL);
	}
	cr_assert(m.curr_stored == NUM_MAP_TEST_ENTRIES,
		"Map does not report the same number of entries as were entered, expected %d but got %d", NUM_MAP_TEST_ENTRIES,
		m.curr_stored);
	...
	for (size_t i = 0; i < NUM_MAP_TEST_ENTRIES; i++)
	{
		...
		cr_assert(
			(size_t)r.just == i * i, "Map did not return correct value, expected %ld but got %ld", i, (size_t)r.just);
		dest_maybe(&r, NULL);
	}
	dest_map(&m, NULL);
}
...
Test(map, push_get_present)
{
	...
	for (size_t i = 0; i < NUM_MAP_TEST_ENTRIES; i++)
	{
		...
		cr_assert((size_t)r.just == (i ? val * val : valv),
			"Pushing present value did not return original value, expected %ld but got %ld instead", valv, r.just);
...
Test(map, iter)
{
	...
	for (size_t i = 1; i <= 4; i++)
	{
		...
		POINTER_TO_INT_CAST(cr_assert(1 <= (int)p->p0 && (int)p->p0 <= 4,
			"Iter returned key which was never in the map, expected e ∈ [1,4] but got e = %d", (int)p->p0));
...
Test(map, iter_keys)
{
	...
	for (size_t i = 1; i <= 4; i++)
	{
		...
		cr_assert(1 <= k && k <= 4,
			"Value iter did not return a value present in the map, expected one of 2,4,6,8 but got %ld", k);
	}
...
Test(map, iter_values)
{
	...
	for (size_t i = 1; i <= 4; i++)
	{
		...
		cr_assert(2 <= v && v <= 8 && v % 2 == 0,
			"Value iter did not return a value present in the map, expected one of 2,4,6,8 but got %ld", v);
	}
