...
-- Modified version of the bin2c.lua found at http://lua-users.org/wiki/BinToCee
...
local src_filename = arg[2]:gsub('.moon$', '.lua')
local status = arg[3]
...
local dump do
  local numtab={}; for i=0,255 do numtab[string.char(i)]=("0x%02x,"):format(i) end
  function dump(str)
...
/* code automatically generated by bin2c -- DO NOT EDIT */
{
	...
	*/
	...
	if (%sluaL_loadbuffer(L,(const char*)B1, sizeof(B1), %q)%s == 0)
	{
		log_debug("Executing lua buffer");
		if (%slua_pcall(L, 0, 0, 0))
		{
			log_err("Running lua buffer failed with error %%s", lua_tostring(L, -1));
			rc = 1;
		}
	}
	else
